...
namespace SalesWebMvc.Data
{
    ...
    {
        /*registrar uma dependencia com o dbcontext(classe salesWebMvcContext)
         * ou seja: qnd o seedingService for criado, automaticamente recebe uma 
         * instancia do context tbm
         */

        private SalesWebMvcContext _context;

        //criar construtor p indicar a injeção de dependencia 
        public SeedingService(SalesWebMvcContext context)
        {
            _context = context;//inje. depend
        }

        //operação responsavel por popular(preencher) a base de dados
        public void Seed()
        {
            //testo se ja existe algum dado

            if (_context.Department.Any() || //algum dado em department
                _context.Seller.Any() ||    //algum dado em seller
                _context.SalesRecord.Any()) //algum dado em salesrecord
            {
                return;// caso exista, retorna esses dados e termina o método
            }

            /*caso n tenha dados:
             * 1° faz a instanciação dos obj
             * 2° manda salvar no bd
             */

            //departamentos(departments):
            Department d1 = new Department(1, "Computers");
            Department d2 = new Department(2, "Eletronis");
            Department d3 = new Department(3, "Fashion");
            Department d4 = new Department(4, "Books" );//só p mostrar que pode ser criado assim tb

            //vendedores(sellers):obs.: d1=> departamento
            ...
            //mandar add no bd:(obs.: addrange pertmite que eu add varios obj de uma só vez)
            //department:
            _context.Department.AddRange(d1, d2, d3, d4);//4 depart

            //sellers:
            _context.Seller.AddRange(s1, s2, s3, s4, s5, s6);//6 vendedores
            ...
            _context.SaveChanges();//salva e confirma as alterações no bd.
        }
    }
}
